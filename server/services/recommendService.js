...
// Given the user's wardrobe, current weather, and preferences, build 1-3 outfits.
...
function currentSeason() {
  const month = new Date().getMonth(); // 0-11, fine to use real Date here (server runtime, not a workflow script)
...
export function recommendOutfits({ wardrobe, weather, preferences }, count = 3) {
  ...
  const byCategory = (cat) => scored.filter((i) => i.category === cat);
  ...
  const wantOuterwear = needsOuterwear || weather.isRainy || weather.isWindy;

  const outfits = [];
  for (let i = 0; i < count; i++) {
    const top = tops[i] || tops[0];
    const bottom = bottoms[i] || bottoms[0];
    if (!top || !bottom) break; // not enough wardrobe variety to build another distinct outfit
    ...
    // avoid building duplicate outfits when wardrobe is small
